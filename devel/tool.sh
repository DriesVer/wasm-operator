...
get_architecture() {
  ...
  case "$arch" in
      ...
      armv* | aarch64 )
          ...
          ;;
      * )
          # Unknown architecture, default to x86
          arch_name="amd64"
          ;;
  esac
}
...
get_architecture ARCH ARCH_NAME
echo ">> Checking tools for architecture: ${ARCH} (${ARCH_NAME})"
...
check_tool_kind() {
  executable_exist kind || {
    curl -Lo "${TOOLS_PATH}/kind" "https://kind.sigs.k8s.io/dl/v0.17.0/kind-linux-${ARCH_NAME}"
...
check_tool_kubectl() {
  executable_exist kubectl || {
    curl -Lo "${TOOLS_PATH}/kubectl" "https://dl.k8s.io/release/v1.24.0/bin/linux/${ARCH_NAME}/kubectl"
...
check_tool_helm() {
  executable_exist helm || {
    ...
    curl -Lo helm.tar.gz "https://get.helm.sh/helm-v3.8.2-linux-${ARCH_NAME}.tar.gz"

    tar -zxvf helm.tar.gz

    mv ./linux-${ARCH_NAME}/helm "./helm"
...
check_tool_kube-apiserver_and_etcd() {
  (
    ...
    curl -Lo kubebuilder-tools.tar.gz https://storage.googleapis.com/kubebuilder-tools/kubebuilder-tools-1.23.6-linux-${ARCH_NAME}.tar.gz
...
check_tool_sccache() {
  executable_exist sccache || {
    pushd "${TOOLS_PATH}"
    curl -Lo sccache.tar.gz https://github.com/mozilla/sccache/releases/download/v0.2.15/sccache-v0.2.15-${ARCH}-unknown-linux-musl.tar.gz

    tar -zxvf sccache.tar.gz

    mv ./sccache-v0.2.15-${ARCH}-unknown-linux-musl/sccache "./sccache"

    rm -rf ./sccache-v0.2.15-${ARCH}-unknown-linux-musl/
...
check_tool_rust() {
  executable_exist rustup || {
    ...
    # maybe add to path $HOME/.cargo/bin"??? (=restart shell)
    source $HOME/.cargo/env
  }

  rustup target add wasm32-wasip1
  # Can result in out of memory if RAM<4GB fix with -j 1 to limit to 1 job
...
check_tool_go() {
  executable_exist go || {
    pushd "${TOOLS_PATH}"
    curl -Lo go1.19.5.linux-${ARCH_NAME}.tar.gz https://go.dev/dl/go1.19.5.linux-${ARCH_NAME}.tar.gz

    sudo rm -rf /usr/local/go
    sudo tar -C /usr/local -xzf go1.19.5.linux-${ARCH_NAME}.tar.gz

    rm -rf go1.19.5.linux-${ARCH_NAME}.tar.gz
    popd
  }
}
...
check_tool_wasm-opt() {
  executable_exist wasm-opt || {
    ...
    curl -Lo binaryen-version_120_b.tar.gz https://github.com/WebAssembly/binaryen/releases/download/version_120_b/binaryen-version_120_b-${ARCH}-linux.tar.gz

    tar -zxvf binaryen-version_120_b.tar.gz

    mv ./binaryen-version_120_b/bin/wasm-opt ./wasm-opt

    rm -rf ./binaryen-version_120_b/
    rm -rf binaryen-version_120_b.tar.gz
    popd
  }
}
...
check_tool_musl-tools() {
  if [ "$ARCH" == "aarch64" ]; then
    if !(apt list --installed 2>/dev/null | grep -q musl-tools); then
      echo ">> Installing musl-tools for aarch64"
...
check_tool_cross() {
  executable_exist cross || {
    ...
    if [ "$ARCH_NAME" != "amd64" ]; then
      # Install x86_64 emulation using qemu if not already enabled
      EMULATOR_CHECK="/proc/sys/fs/binfmt_misc/qemu-x86_64"
      if ! grep -q "enabled" "$EMULATOR_CHECK" 2>/dev/null; then
        echo ">> x86_64 emulation not found. Installing qemu interpreters..."
...
# Small utility to run a command and display its output in a fixed area of the terminal,
# updating in real-time, and showing the last few lines upon completion or error.
# This won't work with grep commands
run() {
    local cmd_pid
    local exit_code
    local output_lines
    local lines_to_show=5
    local logfile
    logfile=$(mktemp)

    # Ensure cleanup on interrupt
    trap 'rm -f "$logfile"; tput cnorm; return' INT

    # Run the command in background, redirecting output to logfile
    echo -e "\033[33m>> Running command: $*\033[0m"
    "$@" > "$logfile" 2>&1 &
    cmd_pid=$!

    # Clear screen area and hide cursor
    tput civis
    for ((i=0; i<lines_to_show; i++)); do echo ""; done

    # Show command output
    while kill -0 "$cmd_pid" 2>/dev/null; do
        tput cuu "$lines_to_show"
        tail -n "$lines_to_show" "$logfile" | \
        awk -v height="$lines_to_show" '{print $0; count++} END {while(count<height){print ""; count++}}' | \
    ...
    tput cuu "$lines_to_show"
    for ((i=0; i<lines_to_show; i++)); do echo -e "\033[K"; done
    tput cuu "$lines_to_show" # Return to start position
